...
const fileFilter = (req, file, cb) => {
  ...
  if (
    ...
    cb(
      new Error(
        "Invalid file type. Only PDF, DOC, DOCX, PPT, PPTX, ZIP, RAR, JPG, JPEG, PNG, GIF, TXT, JS, CSS, HTML and JSON files are allowed.",
      ),
      false,
    );
  }
...
const upload = multer({
  storage,
  fileFilter,
  limits: { fileSize: 10 * 1024 * 1024, files: 10 }, // 10 MB limit
});
...
const handleUploadError = (err, req, res, next) => {
  if (err instanceof multer.MulterError) {
    ...
    if (err.code === "LIMIT_FILE_COUNT") {
      return res.status(400).json({
        success: false,
        error: "To many files the ,Maximum limit of files is 10 .",
      });
    }
