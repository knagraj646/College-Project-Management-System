...
//import { createDeadline } from "../../../../server/controllers/deadlineController";
...
const DeadlinesPage = () => {
  ...
  const projectRows = useMemo(() => {
    return (viewProjects || []).map((p) => ({
      ...
      studentDepartment: p.student?.department || "-",
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!selectedProject || !formData.deadlineData) return;
    let deadlineData = {
      name: selectedProject?.student?.name,
      dueDate: formData.deadlineData,
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-3xl mx-4 max-h-screen overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end space-x-3 tp-4">
                  <button
                    type="button"
                    onClick={() => setShowModal(false)}
