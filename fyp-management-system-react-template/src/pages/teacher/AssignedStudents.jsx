...
//import { markCompelte } from "../../../../server/services/projectService";
...
const AssignedStudents = () => {
  ...
  const confirmMarkComplete = () => {
    if (selectedStudent?.project?._id) {
      dispatch(markCompelte(selectedStudent?.project?._id));
      closeModal();
    }
  };
  ...
  const sortedStudents = [...(assignedStudents || [])].sort((a, b) => {
    switch (sortBy) {
      case "name":
        ...
        break;
      case "lastActivity":
        return new Date(b.project?.updateAt) - new Date(a.project.updateAt);
  ...
  if (loading) {
    return <Loader className="w-16 h-16 animate-spin" />;
  }
  if (error) {
    return (
    ...
    text-sm rounded-lg transition ${
      student?.project?.status === "complete"
        ...
        {showFeedbackModal && selectedStudent && (
          <div
            className="fixed inset-0 bg-blackbg-opacity-50 flex items-center justify-center z-50"
            onClick={closeModal}
          >
            <div
            ...
            >
              <div className="p-6">
                ...
                <div className="bg-slate-50 rounded-lg p-4 mb-6">
                  <div className="space-y-2 text-sm">
                    <div>
                      <span className="font-medoium text-slate-600">
                        Student
                      </span>
                      <span className="ml-2 text-slate-800">
                        {selectedStudent.name}
